...
const DEFAULT_STORE = {
  ...
  customBoosts: [],      // Custom boosting — admin names the boost + price (5 slots) — admin adds camo names, customer picks
  paymentRequests: [],
  tickets: [],
  logs: [],
  settings: {
    ...
    boostingChannelId: '',   // channel for Boosting Services    customBoostingChannelId: '', // channel for BOOSTING section posts posts (fallback: accountsChannelId)
...
// ===== CUSTOM BOOSTING (BOOSTING section — admin names the boost + price, 5 slots) =====
...
client.on('interactionCreate', async (interaction) => {
  ...
  try {
    ...
    if (interaction.isStringSelectMenu() && interaction.customId.startsWith('cbslot_')) {
      ...
      const boost = store.customBoosts.find(s => s.id === boostId);
      if (!boost) return interaction.reply({ content: '❌ Service missing.', ephemeral: true });
      const slot = boost.slots[parseInt(interaction.values[0])];
      ...
      const row = new ActionRowBuilder().addComponents(
        new ButtonBuilder().setCustomId('cbconfirm_' + boostId + '_' + parseInt(interaction.values[0]) + '_' + price).setLabel('✅ تأكيد').setStyle(ButtonStyle.Success),
        new ButtonBuilder().setCustomId('bstcancel').setLabel('❌ إلغاء').setStyle(ButtonStyle.Danger)
      );
      return interaction.update({ embeds: [embed], components: [row] });
    }
