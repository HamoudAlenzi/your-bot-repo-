...
// === MULTI-IMAGE STACKED EMBEDS (1 message, 1 main embed + 1 embed per extra image) ===
async function postAccountToDiscord(channel, acc) {
  ...
  // Build main embed with all info + cover image
  ...
  // Build attachments (cover + extras)
  const files = [];
  ...
  for (let i = 0; i < allImages.length; i++) {
    ...
    if (!parsed) continue;
    const fileName = i === 0 ? `cover_${acc.id}.jpg` : `img_${acc.id}_${i}.jpg`;
    files.push(new AttachmentBuilder(parsed.buffer, { name: fileName }));

    if (i === 0) {
      mainEmbed.setImage('attachment://' + fileName);
    } else {
      // Each additional image goes into its own embed, stacked visually under main
      const imgEmbed = new EmbedBuilder()
        .setColor(brandColor)
        .setImage('attachment://' + fileName)
        .setFooter({ text: 'Image ' + (i + 1) + ' / ' + allImages.length + ' • ' + store.settings.storeName });
  ...
  saveStore();
  addLog('INFO', `Posted ${acc.titleEn} to Discord — ${allImages.length} image(s) in 1 message`);
}
