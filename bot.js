...
app.post('/api/boosting', async (req, res) => {
  try {
    const { titleEn, titleAr, game, serviceType, fromRank, toRank, prestigeLevel, gunName, fromLevel, toLevel,
      price, eta, descriptionEn, descriptionAr, images } = req.body;
    if (!titleEn || !price) return res.status(400).json({ error: 'Title and price required' });
    const service = {
      id: genId(), titleEn, titleAr: titleAr || '',
      game: game || 'Call of Duty',         // Call of Duty / Call of Duty Warzone / etc
...
async function postBoostingToDiscord(channel, service) {
  ...
  mainEmbed.addFields({ name: '💰 Price', value: '```fix\n' + cur + service.price.toFixed(2) + '```', inline: false });
  mainEmbed.setFooter({ text: store.settings.storeName + ' • Boosting Service • ID: ' + service.id });
...
client.on('interactionCreate', async (interaction) => {
  ...
  try {
    ...
    // ---- ORDER BOOST BUTTON (boosting service) ----
    if (interaction.isButton() && interaction.customId.startsWith('orderboost_')) {
      ...
      const existing = store.tickets.find(t =>
        t.userId === interaction.user.id && t.boostingServiceId === boostId && t.status !== 'closed'
      );
      if (existing) {
        const ch = client.channels.cache.get(existing.channelId);
        if (ch) return interaction.reply({ content: `🎫 لديك تذكرة مفتوحة: <#${existing.channelId}>`, ephemeral: true });
      ...
      const ticketChannel = await guild.channels.create({
        name: `🚀-${interaction.user.username}-boost${boostId}`,
        type: ChannelType.GuildText,
        parent: category,
        permissionOverwrites: [
          { id: guild.id, deny: [PermissionFlagsBits.ViewChannel] },
      ...
      const ticket = {
        id: ticketId, userId: interaction.user.id, userName: interaction.user.username,
        accountId: null, poolId: null, digitalProductId: null, boostingServiceId: boostId,
        accountTitle: boost.titleEn + ' (Boosting)', amount: boost.price,
      ...
      const selectMenu = new StringSelectMenuBuilder()
        .setCustomId('paymethodboost_' + boostId + '_' + ticketId)
        .setPlaceholder('اختر طريقة الدفع / Choose payment')
        .addOptions(options);
      ...
      const embed = new EmbedBuilder()
        ...
        .setDescription(
          ...
          `💰 السعر: \`${pay.currency}${boost.price.toFixed(2)}\`\n` +
          `⏱️ ETA: \`${boost.eta}\`\n` +
          `🎫 التذكرة: \`${ticketId}\`\n\n` +
      ...
      await interaction.reply({ content: '🎫 تم إنشاء تذكرة: <#' + ticketChannel.id + '>', ephemeral: true });
      addLog('INFO', `Boosting ticket ${ticketId} for ${interaction.user.username} → ${boost.titleEn}`);
      sendLogToDiscord(`🚀 Boosting ticket \`${ticketId}\` by **${interaction.user.username}** for **${boost.titleEn}** ($${boost.price})`);
      return;
    }
    ...
    if (interaction.isStringSelectMenu() && interaction.customId.startsWith('paymethodboost_')) {
      ...
      const method = interaction.values[0];
      const payId = createPaymentRequest(interaction, null, boost.titleEn + ' (Boosting)', boost.price, method, ticketId, null, null, boostId);
      const pay = store.settings;
      let textInfo = '';
      if (method === 'stcpay') textInfo = `📱 **STC Pay**\nالرقم: \`${pay.stcPay.number}\`\nالاسم: *${pay.stcPay.name || '-'}*`;
      if (method === 'alrajhi') textInfo = `🏦 **AlRajhi Bank**\nIBAN: \`${pay.alrajhi.iban}\`\nالاسم: *${pay.alrajhi.name || '-'}*`;
      ...
      const payEmbed = new EmbedBuilder()
        ...
        .setDescription(
          `**الخدمة:** ${boost.titleEn} (${boost.game})\n` +
          `**المبلغ:** \`${pay.currency}${boost.price.toFixed(2)}\`\n` +
          `**ETA:** \`${boost.eta}\`\n` +
          `**رقم العملية:** \`${payId}\`\n\n` +
          textInfo + '\n\n' +
