...
// === MULTI-IMAGE GRID LAYOUT (1 embed with text + buttons + N image attachments auto-gridded below by Discord) ===
async function postAccountToDiscord(channel, acc) {
  ...
  // ===== SINGLE EMBED with all info (NO setImage — images go as grid below) =====
  // This produces the layout the user wants: 1 embed (text + buttons) with all
  // product images displayed as a grid of attachments below the embed.
  // Discord auto-arranges image attachments in a 2x2 grid for 4 images,
  // side-by-side for 2, full-width for 1, etc.
  ...
  // ===== Build image attachments (sent as plain files, Discord auto-grids them) =====
  const files = [];
  ...
  // Send: 1 embed (text only) + N image files (auto-gridded by Discord below the embed)
  const msg = await channel.send({ embeds: [mainEmbed], components: [row], files });
  acc.discordMessageIds.push(msg.id);
  saveStore();
  addLog('INFO', `Posted ${acc.titleEn} to Discord — ${allImages.length} image(s) as grid below embed`);
}
