...
// Features: BO7 & Warzone focused — Multi-image stacked embeds, private tickets, payment flow,
...
const STORE_TAGLINE = 'Black Ops 7 & Warzone — Premium Store';
...
// ===== BLACK OPS 7 + WARZONE GUN & CAMO DATABASE =====
const BO7_GUNS = {
  'Assault Rifles': [
    'AK-74','XM4','AK-47','G36C','Krig 6','STG44','FARA 83','EM2','QBZ-83',
    'LK24','SVA 545','KVD Enforcer','TAR-21','AEK-971','AS VAL','Grau 5.56'
  ],
  'Submachine Guns': [
    'MP5','KSP 45','MP7','LC10','Bullfrog','OTs 9','MAC-10','PP19 Bizon',
    'Kompakt 92','C9','JS9','Superi 46','XM10','VMP','UGR','Striker'
  ],
  'Tactical Rifles': [
    'AUG','M16','CARV.2','DMR 14','Type 63','EBR-14','SVD','SP-R 208','MK2 Carbine'
  ],
  'Light Machine Guns': [
    'PKM','Stoner 63','RPK','M60','HAMR','MG36','DP27','Bruen MK9','FiNN LMG'
  ],
  'Shotguns': [
    'Olympia','SPAS-12','Model 680','GALLO SA12','Hauer 77','Jak-12','Haymaker','Streetsweeper'
  ],
  'Sniper Rifles': [
    'Pelington 703','M40A3','LW3 Tundra','Swiss K31','HDR','ZRG 20mm','TAC-50','Dragunov','AX-50'
  ],
  'Pistols': [
    '1911','Magnum','X16','GS45','Marshal','Dual Kamas','Renetti','Sykov'
  ],
  'Launchers': [
    'RPG-7','Cigma 2','PILA','STRELA-P','JOKR'
  ],
  'Melee': [
    'Knife','Karambit','Sledgehammer','Nunchucks','Machete','Baseball Bat'
  ]
};

const BO7_CAMOS = [
  { name: 'Military', emoji: '🟢', category: 'base' },
  { name: 'Woodland', emoji: '🟤', category: 'base' },
  { name: 'Splinter', emoji: '⬛', category: 'base' },
  { name: 'Digital', emoji: '🔴', category: 'base' },
  { name: 'Dragon', emoji: '🐉', category: 'base' },
  { name: 'Blossom', emoji: '🌸', category: 'base' },
  { name: 'Topo', emoji: '🗺️', category: 'base' },
  { name: 'Striped', emoji: '🦓', category: 'base' },
  { name: 'Tartan', emoji: '🔷', category: 'base' },
  { name: 'Sandstorm', emoji: '🏜️', category: 'base' },
  { name: 'Sakura', emoji: '🏵️', category: 'base' },
  { name: 'Jade', emoji: '💚', category: 'base' },
  { name: 'Gold', emoji: '🥇', category: 'mastery', price: 5.00 },
  { name: 'Diamond', emoji: '💎', category: 'mastery', price: 15.00 },
  { name: 'Dark Matter', emoji: '💜', category: 'mastery', price: 35.00 },
  { name: 'Dark Aether', emoji: '🌌', category: 'mastery', price: 50.00 },
  { name: 'Nebula', emoji: '🔮', category: 'seasonal', price: 45.00 },
  { name: 'Obsidian', emoji: '🖤', category: 'seasonal', price: 55.00 },
  { name: 'Catalyst', emoji: '⚡', category: 'seasonal', price: 60.00 },
  { name: 'Abyssal', emoji: '🌊', category: 'seasonal', price: 65.00 },
  { name: 'Magma', emoji: '🌋', category: 'seasonal', price: 70.00 },
  { name: 'Frost', emoji: '❄️', category: 'seasonal', price: 70.00 }
];

const BO7_ALL_GUNS_FLAT = Object.values(BO7_GUNS).flat();

function getBO7GunListForCamo() {
  return BO7_ALL_GUNS_FLAT.map(g => ({ name: g, emoji: '🔫' }));
}

function getBO7CamoList(pricePerCamo) {
  return BO7_CAMOS.map(c => ({
    name: c.name,
    emoji: c.emoji,
    price: c.price || pricePerCamo
  }));
}
  ...
  try {
    ...
    if (ticket && ticket.channelId && client.isReady()) {
      ...
      if (ticketChannel) {
        let deliverEmbed;
        if (isCamo) {
          // CAMO: payment confirmed, ask for account credentials, mark ticket camo_in_progress
          ...
          deliverEmbed = new EmbedBuilder()
            ...
            .setTitle('✅ تم تأكيد الدفع — فتح الكاموهات قيد التنفيذ / Payment Confirmed — Camo Unlock In Progress')
            .setDescription(
              `**${pr.accountTitle}**\n` +
              `💰 المبلغ المدفوع: \`${store.settings.currency}${finalAmount.toFixed(2)}\`\n` +
              `🎫 رقم العملية: \`${pr.id}\`\n\n` +
              `📋 **الخطوة التالية / Next Step:**\n` +
              `أرسل بيانات حسابك هنا (Email + Password + أي معلومات مطلوبة) لبدء فتح الكاموهات.\n` +
              `Send your account credentials here (Email + Password + any required info) to start camo unlock.\n\n` +
              `⏱️ ETA: \`${camoSvc ? camoSvc.eta : '24-72 hours'}\`\n` +
              `🎨 سيتم إشعارك فور اكتمال فتح الكاموهات!`
            )
            .setFooter({ text: store.settings.storeName + ' • Ticket stays open until camo unlock completes' })
            .setTimestamp();
      ...
      if (pr.userId && client.isReady()) {
        client.users.fetch(pr.userId).then(user => {
          if (isCamo) {
            user.send(`✅ **${store.settings.storeName} — Payment Confirmed**\n\n**${pr.accountTitle}**\nYour camo unlock is now in progress. Send your account credentials in the ticket.\n⏱️ ETA: 24-72 hours`).catch(() => {});
          } else if (isBoosting) {
...
app.post('/api/boosting', async (req, res) => {
  try {
    ...
    const service = {
      ...
      gunList: Array.isArray(gunList) ? gunList.map(parseGunEntry) : (gunList ? gunList.map(parseGunEntry) : []),
...
async function postBoostingToDiscord(channel, service) {
  ...
  if (service.serviceType === 'gun_level' && service.gunList && service.gunList.length > 0) {
    ...
    const gunsText = service.gunList.map(g => (g.emoji ? g.emoji + ' ' : '') + '`' + g.name + '` (' + cur + (g.pricePerLevel||0) + '/lvl)').join(', ');
  ...
  try {
    ...
    if (ticket.channelId && client.isReady()) {
      ...
      if (ch) {
        const embed = new EmbedBuilder()
          .setColor(store.settings.color || 0x9b59ff)
          .setTitle('✅ تم الانتهاء من فتح الكاموهات / Camo Unlock Complete!')
          .setDescription(
            `**${ticket.accountTitle}**\n` +
            `🎨 تم فتح جميع الكاموهات المطلوبة بنجاح! حسابك جاهز الآن.\n` +
            `All requested camos have been unlocked — your account is ready!\n\n` +
            `🙏 شكراً لثقتك في **${store.settings.storeName}**!`
          )
          .setFooter({ text: store.settings.storeName + ' • Closing in ' + (store.settings.autoCloseSeconds || 15) + 's' })
          .setTimestamp();
...
// ===== BO7 QUICK SETUP ENDPOINTS =====
app.get('/api/bo7/data', (req, res) => {
  try {
    res.json({
      guns: BO7_GUNS,
      allGuns: BO7_ALL_GUNS_FLAT,
      camos: BO7_CAMOS,
      gunCategories: Object.keys(BO7_GUNS)
    });
  } catch (e) { res.status(500).json({ error: e.message }); }
});

app.post('/api/bo7/setup-camo', async (req, res) => {
  try {
    const { pricePerCamo, eta, customTitle, postToDiscord } = req.body;
    const ppc = parseFloat(pricePerCamo) || 3.00;
    const titleEn = customTitle || 'Black Ops 7 — Full Camo Unlock';
    const titleAr = customTitle ? '' : 'بلاك أوبس 7 — فتح جميع الكاموهات';
    const camoList = getBO7CamoList(ppc);
    const gunList = getBO7GunListForCamo();

    // Check if BO7 camo service already exists
    const existing = store.camoServices.find(s => s.titleEn.includes('Black Ops 7') && s.titleEn.includes('Camo'));
    if (existing) {
      // Update existing
      existing.camoList = camoList;
      existing.gunList = gunList;
      existing.pricePerCamo = ppc;
      existing.eta = eta || existing.eta || '24-72 hours';
      existing.game = 'Black Ops 7 / Warzone';
      if (customTitle) { existing.titleEn = titleEn; existing.titleAr = titleAr; }
      saveStore();
      addLog('INFO', `BO7 Camo service updated (ID: ${existing.id}) with ${camoList.length} camos, ${gunList.length} guns`);
      return res.json({ success: true, action: 'updated', service: existing });
    }

    const service = {
      id: genId(), titleEn, titleAr,
      game: 'Black Ops 7 / Warzone',
      pricePerCamo: ppc,
      camoList, gunList,
      eta: eta || '24-72 hours',
      descriptionEn: 'Full camo unlock for any BO7 or Warzone gun. Pick your gun, pick your camos — we unlock them all!',
      descriptionAr: 'فتح جميع الكاموهات لأي سلاح في بلاك أوبس 7 أو وارزون. اختر السلاح والكاموهات!',
      images: [],
      status: 'active',
      createdAt: new Date().toISOString(),
      discordMessageIds: []
    ...
    if (postToDiscord !== false) {
      const channelId = store.settings.camoChannelId || store.settings.accountsChannelId;
      if (channelId && client.isReady()) {
        const channel = client.channels.cache.get(channelId);
        if (channel) await postCamoToDiscord(channel, service).catch(e => addLog('ERROR', 'BO7 camo post failed: ' + e.message));
      }
    }

    addLog('INFO', `BO7 Camo service created (ID: ${service.id}) with ${camoList.length} camos, ${gunList.length} guns`);
    res.json({ success: true, action: 'created', service });
  } catch (e) { res.status(500).json({ error: e.message }); }
});

app.post('/api/bo7/setup-boosting', async (req, res) => {
  try {
    const { serviceType, pricePerUnit, eta, maxLevel, customTitle, postToDiscord } = req.body;
    const titleEn = customTitle || 'Black Ops 7 — Gun Leveling Service';
    const titleAr = customTitle ? '' : 'بلاك أوبس 7 — تسوية أسلحة';
    const ppu = parseFloat(pricePerUnit) || 0.50;
    const maxLvl = parseInt(maxLevel) || 100;
    const sType = serviceType || 'gun_level';

    const gunList = BO7_ALL_GUNS_FLAT.map(g => ({
      name: g, emoji: '🔫', pricePerLevel: ppu
    }));

    // Check if BO7 boosting service already exists
    const existing = store.boostingServices.find(s => s.titleEn.includes('Black Ops 7') && s.titleEn.includes('Gun Level'));
    if (existing && sType === 'gun_level') {
      existing.gunList = gunList;
      existing.maxGunLevel = maxLvl;
      existing.price = ppu;
      existing.eta = eta || existing.eta || '12-48 hours';
      existing.game = 'Black Ops 7 / Warzone';
      if (customTitle) { existing.titleEn = titleEn; existing.titleAr = titleAr; }
      saveStore();
      addLog('INFO', `BO7 Boosting service updated (ID: ${existing.id})`);
      return res.json({ success: true, action: 'updated', service: existing });
    }
    ...
    const service = {
      ...
      game: 'Black Ops 7 / Warzone',
      serviceType: sType,
      gunName: '', fromLevel: '', toLevel: '',
      price: ppu,
      eta: eta || '12-48 hours',
      descriptionEn: 'Level up any BO7 or Warzone gun to max! All guns available.',
      descriptionAr: 'سوّي أي سلاح في بلاك أوبس 7 أو وارزون للحد الأقصى! جميع الأسلحة متاحة.',
      images: [],
      rankList: [], rankTiers: 3, pricePerRank: 0,
      maxPrestige: 0, pricePerPrestige: 0, prestigeEmoji: '🎖️',
      gunList, maxGunLevel: maxLvl,
      status: 'active',
      createdAt: new Date().toISOString(),
      discordMessageIds: []
    };
    ...
    if (postToDiscord !== false) {
      ...
      if (channelId && client.isReady()) {
        const channel = client.channels.cache.get(channelId);
        if (channel) await postBoostingToDiscord(channel, service).catch(e => addLog('ERROR', 'BO7 boost post failed: ' + e.message));
      }
    }

    addLog('INFO', `BO7 Boosting service created (ID: ${service.id}) with ${gunList.length} guns`);
    res.json({ success: true, action: 'created', service });
  } catch (e) { res.status(500).json({ error: e.message }); }
});

app.post('/api/bo7/setup-pool', async (req, res) => {
  try {
    const { name, price, description, game, postToDiscord } = req.body;
    const poolName = name || 'BO7 Accounts Pool';
    const pool = {
      id: genId(), name: poolName,
      description: description || 'Auto-delivery Black Ops 7 accounts. Instant delivery after payment!',
      price: parseFloat(price) || 10.00,
      game: game || 'Black Ops 7',
      image: null, stock: [],
      createdAt: new Date().toISOString(), discordMessageIds: []
    };
    store.pools.push(pool);
    ...
    if (postToDiscord !== false) {
      ...
      if (channelId && client.isReady()) {
        const channel = client.channels.cache.get(channelId);
        if (channel) await postPoolToDiscord(channel, pool).catch(e => addLog('ERROR', 'BO7 pool post failed: ' + e.message));
      }
    }

    addLog('INFO', `BO7 Pool created: ${poolName}`);
...
app.get('/api/health', (req, res) => {
  res.json({
    ...
    storeSize: Math.round(JSON.stringify(store).length / 1024) + 'KB',
    pendingPayments: store.paymentRequests.filter(p => p.status === 'Pending' || p.status === 'Waiting Review').length,
    openTickets: store.tickets.filter(t => t.status !== 'closed').length
  });
});
...
setInterval(() => {
  const now = Date.now();
  for (const [sid, exp] of SESSIONS) {
    if (now > exp) SESSIONS.delete(sid);
  }
}, 3600000);
...
client.on('interactionCreate', async (interaction) => {
  ...
  try {
    ...
    if (interaction.isButton() && interaction.customId.startsWith('orderboost_')) {
      ...
      // ===== GUN LEVEL with customer choice (with emojis) =====
      if (boost.serviceType === 'gun_level' && boost.gunList && boost.gunList.length > 0) {
        const options = boost.gunList.slice(0, 25).map((g, i) => ({ label: g.name.slice(0, 100), value: String(i), description: cur + (g.pricePerLevel||0) + ' per level', emoji: g.emoji || undefined }));
    ...
    if (interaction.isStringSelectMenu() && interaction.customId.startsWith('bstg_')) {
      ...
      const modal = new ModalBuilder().setCustomId('bstgm_' + boostId + '_' + gunIdx).setTitle(gunEmoji + gun.name + ' — Enter Levels');
...
async function createCamoTicket(interaction, camo, gunIdx, selectedCamos, price) {
  ...
  const embed = new EmbedBuilder()
    .setColor(store.settings.color || 0x9b59ff)
    .setTitle('🎨 طلب فتح كاموهات / Camo Unlock Request')
    .setDescription(
      `**${store.settings.storeName}**\n\n` +
      `👤 العميل: **${interaction.user.username}**\n` +
